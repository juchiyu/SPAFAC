...
# run sDiCA
sdica.wine.res <- sparseDiCA(as.matrix(X.data), design = X.des, make_data_nominal = FALSE, make_design_nominal = FALSE, components = 2L, rdsLeft = rep(sqrt(nrow(X.dica)), 2), rdsRight = rep(sqrt(ncol(X.dica)), 2))
